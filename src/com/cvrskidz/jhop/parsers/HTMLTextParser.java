...
public class HTMLTextParser extends HTMLParser<String>{
    ...
    public String output() {
        ...
        while(it.hasNext()) {
            ...
            if(HTMLParser.WIDTH > 0) out.append(wrap(current.text()));
            else out.append(current.text());
    ...
    /**
     * Wraps a line of text so that the returned string fits within the size
     * specified by HTMLParser.WIDTH.
     * 
    ...
    private String wrap(String line) {
        StringBuilder out = new StringBuilder();
        String currentLine;
        while(line.length() > HTMLParser.WIDTH) {
            currentLine = line.substring(0, HTMLParser.WIDTH);
            line = line.substring(HTMLParser.WIDTH);
     ...
     * that start with a node included in JHop's taglist or match the given query.
     * <p>
     * For example given the query "class" and "content" the tag "first" and "third"
     * will be parsed, but not "second".
     * <pre>{@code
     *      <div class="content">
     *          <p id="first">...</p>
     *          <div id="pictures">
     ...
     * @return All elements of the matching node branches.
     */
    @Override
    protected Elements allChildrenOf(Element node){
        Elements elements = new Elements();
        for(Element e: node.children()){
            boolean matchesQuery = e.attributes().get(attribute).equals(value);
            boolean isIncluded = tags != null && tags.contains(e.tagName());
            
            // add the current element if included in tag list
            if(isIncluded) elements.add(e); 
            
            // add branch if included in tag list or matches qeury
            if(matchesQuery || isIncluded) elements.addAll(allChildrenOf(e)); 
        }
